...
class B2HadDstDecayTreeProducer : public edm::EDProducer {

public:

    explicit B2HadDstDecayTreeProducer(const edm::ParameterSet &iConfig);
    ...
    ~B2HadDstDecayTreeProducer() {
      cout << Form("Total number of fit crashed %u", fitCrash) << endl;
      cout << Form("B2HadDstDecayTreeProducer counters:%d\n", (int)counters.size());
      for(auto v : counters) {
        cout << v << endl;
      }
      cout << "\n\n";
...
private:
    ...
    edm::EDGetTokenT<vector<pat::PackedCandidate>> TrgCandSrc_;

    int charge_trgCand = +1;
    int charge_K = +1;
    int charge_pi = -1;
    int charge_pis = -1;
...
B2HadDstDecayTreeProducer::B2HadDstDecayTreeProducer(const edm::ParameterSet &iConfig)
{
    ...
    TrgCandSrc_ = consumes<vector<pat::PackedCandidate>> ( iConfig.getParameter<edm::InputTag>( "trgCandidates" ) );

    charge_trgCand = iConfig.getParameter<int>( "charge_trgCand" );
...
void B2HadDstDecayTreeProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
    ...
    edm::Handle<vector<pat::PackedCandidate>> trgCandidatesHandle;
    iEvent.getByToken(TrgCandSrc_, trgCandidatesHandle);

    // Output collection
    unique_ptr<map<string, float>> outputNtuplizer(new map<string, float>);
    unique_ptr<map<string, vector<float>>> outputVecNtuplizer(new map<string, vector<float>>);

    // Creating the list of trigger muon in order to veto it
    if (verbose) {cout <<"-------------------- Evt -----------------------\n";}
    vector<pat::Muon> trgSoftMuons;
    for(auto muon : (*muonHandle)) {
      if(!muon.triggered("HLT_Mu*_IP*_part*_v*")) continue;
      //######## Require muon quality ##################
      if (muon.charge() != charge_trgCand) continue;
      if (muon.innerTrack().isNull()) continue;
      if (fabs(muon.eta()) > 1.5) continue;

      bool softMuon = false;
      for(uint i_vtx = 0; i_vtx<vtxHandle->size(); i_vtx++) {
        auto vtx = (*vtxHandle)[i_vtx];
        if(vtx.normalizedChi2() > 1.5) continue;
        if(muon.isSoftMuon(vtx)){
          softMuon = true;
          break;
        }
      }
      if (softMuon) trgSoftMuons.push_back(muon);
    }

    int n_trgCand = 0, n_K = 0, n_pi = 0, n_D0 = 0, n_pis = 0, n_Dst = 0, n_B = 0;

    vector<bool> countersFlag(counters.size(), false);
    counters[0]++;
    for(uint i_trgCand = 0; i_trgCand < trgCandidatesHandle->size(); i_trgCand++) {
      //######## Require muon quality ##################
      auto trgCand = (*trgCandidatesHandle)[i_trgCand];
      if (trgCand.charge() != charge_trgCand) continue;
      if (!trgCand.hasTrackDetails()) continue;
      if (fabs(trgCand.eta()) > 1.5) continue;
      updateCounter(1, countersFlag);
      if(verbose) {cout << "Passed Charge,Eta and track details requirements for candidate " << i_trgCand << endl;}

      bool matchSoftMuon = false;
      for(auto muon : trgSoftMuons) {
        float dR = vtxu::dR(muon.phi(), trgCand.phi(), muon.eta(), trgCand.eta());
        float dpt = fabs(muon.pt() - trgCand.pt())/trgCand.pt();
        if (dpt < 0.05 && dR < 0.01) {
          matchSoftMuon = true;
          if(verbose) {cout << Form("Candidate match with Soft Muon (dpt/pt=%.4f, dR=%.4f)", dpt, dR) << endl;}
          break;
        }
      }
      if (matchSoftMuon) continue;
      updateCounter(2, countersFlag);

      n_trgCand++;

      // Retireve the index in the PF cand list
      uint idxPFCand_trgCand = 0;
      for(uint i = 0; i < N_pfCand; ++i) {
        const pat::PackedCandidate & p = (*pfCandHandle)[i];
        bool match = p.pt() == trgCand.pt();
        match = match && (p.phi() == trgCand.phi());
        match = match && (p.eta() == trgCand.eta());
        match = match && (p.pdgId() == trgCand.pdgId());
        if (match) {
          idxPFCand_trgCand = i;
          if(verbose) {
            cout << "Matching PF candidate found at idx " << i << endl;
          }
          break;
        }
      }
      ...
      */
      for(uint i_K = 0; i_K < N_pfCand; ++i_K) {
        if (i_K == idxPFCand_trgCand) continue;
        ...
        if (fabs(K_tk->dz(primaryVtx.position()) - trgCand.bestTrack()->dz(primaryVtx.position())) > __dzMax__) continue;
        if (vtxu::dR(K.phi(), trgCand.phi(), K.eta(), trgCand.eta()) > __dRMax__) continue;
        ...
        */
        for(uint i_pi = 0; i_pi < N_pfCand; ++i_pi) {
          if(i_pi==i_K || i_pi==idxPFCand_trgCand) continue;
          ...
          if (fabs(pi_tk->dz(primaryVtx.position()) - trgCand.bestTrack()->dz(primaryVtx.position())) > __dzMax__) continue;
          if (vtxu::dR(pi.phi(), trgCand.phi(), pi.eta(), trgCand.eta()) > __dRMax__) continue;
          // Require significant impact parameter
          ...
          */
          for(uint i_pis = 0; i_pis < N_pfCand; ++i_pis) {
            if(i_pis==i_K || i_pis==i_pi || i_pis==idxPFCand_trgCand) continue;
            ...
            if (fabs(pis_tk->dz(primaryVtx.position()) - trgCand.bestTrack()->dz(primaryVtx.position())) > __dzMax__) continue;
            if (vtxu::dR(pis.phi(), trgCand.phi(), pis.eta(), trgCand.eta()) > __dRMax__) continue;
            // Require significant impact parameter
            ...
            auto BKinTree = vtxu::FitD0pipi(iSetup, D0, pis, trgCand);
            ...
            uint i_best = 0;
            auto maxCos = vtxu::computePointingCos(primaryVtx, vtxB, D0pismu);
            for(uint i_vtx = 1; i_vtx < vtxHandle->size(); i_vtx++) {
              auto auxCos = vtxu::computePointingCos((*vtxHandle)[i_vtx], vtxB, D0pismu);
              if(auxCos > maxCos) {
                maxCos = auxCos;
                i_best = i_vtx;
              }
            ...
            */
            ...
            for(uint i_tk = 0; i_tk < N_pfCand; ++i_tk) {
              ...
              if( i_tk==i_K || i_tk==i_pi || i_tk==i_pis || i_tk==idxPFCand_trgCand) continue;
              ...
              if (fabs(tk->dz(primaryVtx.position()) - trgCand.bestTrack()->dz(primaryVtx.position())) > __dzMax__) continue;
              if (vtxu::dR(ptk.phi(), trgCand.phi(), ptk.eta(), trgCand.eta()) > __dRMax__) continue;
              ...
              auto kinTree = vtxu::FitD0pipipis(iSetup, D0, pis, trgCand, ptk);
            ...
            (*outputVecNtuplizer)["mu_trgCand_idx"].push_back(i_trgCand);
            AddTLVToOut(vtxu::getTLVfromCand(trgCand, mass_pi), string("mu"), &(*outputVecNtuplizer));
            GlobalPoint auxp(vtxDst->position().x(), vtxDst->position().y(), vtxDst->position().z());
            auto dca = vtxu::computeDCA(iSetup, trgCand, auxp);
            (*outputVecNtuplizer)["mu_dca_vtxDst"].push_back(dca.first);
            (*outputVecNtuplizer)["mu_sigdca_vtxDst"].push_back(dca.first/dca.second);
            auto dcaT = vtxu::computeDCA(iSetup, trgCand, auxp, 1);
            ...
            auto dcaL = vtxu::computeDCA(iSetup, trgCand, auxp, 2);
            ...
            dca = vtxu::computeDCA(iSetup, trgCand, auxp2);
            ...
            dcaT = vtxu::computeDCA(iSetup, trgCand, auxp2, 1);
            ...
            dcaL = vtxu::computeDCA(iSetup, trgCand, auxp2, 2);
    ...
    (*outputNtuplizer)["n_trgCand"] = n_trgCand;
...
void B2HadDstDecayTreeProducer::AddTLVToOut(TLorentzVector v, string n, map<string, vector<float>>* outv) {
...
bool B2HadDstDecayTreeProducer::qualityMuonID(pat::Muon m, reco::Vertex pVtx) {
...
void B2HadDstDecayTreeProducer::updateCounter(int idx, vector<bool> &cF) {
  if(!cF[idx]) {
    cF[idx] = true;
    counters[idx]++;
  }
...
double B2HadDstDecayTreeProducer::Mass_varM(TLorentzVector p1, double m1, TLorentzVector p2, double m2){
...
DEFINE_FWK_MODULE(B2HadDstDecayTreeProducer);
