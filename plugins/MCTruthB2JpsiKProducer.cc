...
class MCTruthB2JpsiKProducer : public edm::EDProducer {

public:

    explicit MCTruthB2JpsiKProducer(const edm::ParameterSet &iConfig);
    void AddTLVToOut(TLorentzVector, string, map<string, float>*);

    ~MCTruthB2JpsiKProducer() override {};
...
MCTruthB2JpsiKProducer::MCTruthB2JpsiKProducer(const edm::ParameterSet &iConfig)
{
...
void MCTruthB2JpsiKProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
    ...
    for( auto const& kv : (*outMapHandle) ) {
      if (kv.first == "B_mumuK_pt") reco_B_pt = kv.second;
      if (kv.first == "B_mumuK_eta") reco_B_eta = kv.second;
      if (kv.first == "B_mumuK_phi") reco_B_phi = kv.second;
    }
    if(verbose) {
    ...
    for(unsigned int i = 0; i < N_PrunedGenParticles; i++) {
      ...
      if (abs(p.pdgId()) == 521 && p.numberOfDaughters()>1) {
        bool Jpsi_found = false;
        bool K_found = false;
        for(auto d : p.daughterRefVector()) {
          if(d->pdgId() == 443) Jpsi_found = true;
          else if(abs(d->pdgId()) == 321) K_found = true;
        }
        if(Jpsi_found && K_found) nB02JpsiK++;
        else continue;

        if(verbose) {cout << Form("%d (B0->JpsiK): pt:%.2f eta:%.2f phi:%.2f", i, p.pt(), p.eta(), p.phi()) << endl;}
    ...
    (*indexBmc) = i_B;
    (*outputNtuplizer)["MC_nB02JpsiK"] = nB02JpsiK;
...
void MCTruthB2JpsiKProducer::AddTLVToOut(TLorentzVector v, string n, map<string, float>* outv) {
  (*outv)[n+"_pt"] = v.Pt();
  (*outv)[n+"_eta"] = v.Eta();
  (*outv)[n+"_phi"] = v.Phi();
  return;
...
DEFINE_FWK_MODULE(MCTruthB2JpsiKProducer);
